...
public:
    ...
    void update(int from, int to, int lo, int hi, int root, change delta. int b)
    {
        if(lo>hi) return;
...
public:
    ...
    void update(int from, int to, change delta, int b)
    {
        update(from, to, 0, n-1, 0, delta, b);
    }
...
void solve(int tc)
{
    ...
    SegmentTree<int,pair<bool,int>> st(v, sumVal, 0, applyReplace, mergeReplace, noChange);
    ...
    while(q--)
    {
        ...
        if(t==0)
        {
            ...
            delta={1, c}; // flag=true, value = u
            st.update(l, r-1, delta, b); // from=k, to=k, delta=delta
        }
