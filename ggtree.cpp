...
void lazy_update(int node, int b, int e, int from, int to, int x)
{
    if(lazy[node]!=0)
    {
        cout<<"prev value "<<segtree[node]<<" "<<node<<"\n";
        segtree[node] += (e-b+1)*lazy[node];

        cout<<"updated value "<<segtree[node]<<" "<<node<<"\n";
...
int query(int node, int begin, int end, int from, int to)
{
    if(lazy[node]!=0)
    {
        segtree[node]+= (end-begin+1)*lazy[node];
        if(begin!=end)
        {
            lazy[node*2] += lazy[node];
            lazy[node*2+1] += lazy[node];
